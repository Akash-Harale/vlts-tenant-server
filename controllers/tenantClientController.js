    ...
    try {
      ...
      const result = await session.withTransaction(async () => {
        const {
          entity_name, contact_name, gst_number, cin_number,
          address1, address2, city, district, state, pincode,
          mobile_number, whatsapp_number, email_id,
          admin_email, admin_mobile_number, password
        } = req.body;

        // Required field validation
        if (!entity_name || !contact_name || !gst_number || !cin_number || !email_id) {
          const e = new Error('entity_name, contact_name, gst_number, cin_number and email_id are required');
          e.statusCode = 400;
          throw e;
        }
        if (!admin_email || !password) {
          const e = new Error('admin_email and password are required for client admin login');
          e.statusCode = 400;
          throw e;
        }
        ...
        if (!clientAdminRole) {
          const e = new Error('Role "client_admin" not found — ensure roles are seeded');
          e.statusCode = 500;
          throw e;
        }
      ...
      const message = err.code === 11000 ? 'Duplicate entry (GST/CIN/email already exists)' : err.message;

      await logger.error(meta.emp_id, meta.emp_name, meta.role, err, 'clientCreate', meta.tenant_id, meta.trace_id, statusCode);
...
exports.getClients = async (req, res) => {
  const meta = getMeta(req);
  try {
    const clients = await ClientProfile.find({ tenant_id: req.user.tenant_id })
      .sort({ created_at: -1 });
    ...
    await logger.error(meta.emp_id, meta.emp_name, meta.role, err, 'clientRead', meta.tenant_id, meta.trace_id, 500);
...
exports.updateClient = async (req, res) => {
  ...
  try {
    ...
    if (req.body.entity_name) {
      ...
      session.endSession();
      return res.status(400).json({ success: false, message: 'entity_name cannot be changed after creation' });
    }

    // Whitelist updatable client profile fields
    const allowedFields = [
      'contact_name', 'gst_number', 'cin_number',
      'address1', 'address2', 'city', 'district', 'state', 'pincode',
      'mobile_number', 'whatsapp_number', 'email_id', 'status'
    ];
    const updateFields = {};
    ...
    if (req.body.admin_mobile_number) empUpdate.mobile_number = req.body.admin_mobile_number;
    if (req.body.admin_whatsapp_number) empUpdate.whatsapp_number = req.body.admin_whatsapp_number;

    if (Object.keys(empUpdate).length > 0) {
      await Employee.updateMany({ client_profile_id: clientId }, { $set: empUpdate }, { session });
    }
    if (Object.keys(userUpdate).length > 0) {
      await User.updateMany({ client_profile_id: clientId }, { $set: userUpdate }, { session });
    }
    ...
    await session.abortTransaction();
    session.endSession();
    await logger.error(meta.emp_id, meta.emp_name, meta.role, err, 'clientUpdate', meta.tenant_id, meta.trace_id, 500);
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.deleteClient = async (req, res) => {
  ...
  try {
    ...
    const userCount = await User.countDocuments({ client_profile_id: clientId });
    if (userCount > 0) {
    ...
    await logger.error(meta.emp_id, meta.emp_name, meta.role, err, 'clientDelete', meta.tenant_id, meta.trace_id, 500);
    return res.status(500).json({ success: false, message: err.message });
  }
};
